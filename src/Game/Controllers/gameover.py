...
class Gameover:
    def __init__(self, window, winner):
        ...
        if winner == 'b':
            self.gameover_drawer = GameoverDrawer(self.window, 'Черные победили!')
        elif winner == 'w':
            self.gameover_drawer = GameoverDrawer(self.window, 'Белые победили!')
        else:
            self.gameover_drawer = GameoverDrawer(self.window, 'Ничья!')
