...
class MenuDrawer:
    ...
    def __make_buttons(self):
        ...
        self.singleplayer_button_surface = \
            make_button_surface(self.button_font, self.singleplayer_button, "Битва с ии", BUTTON_COLOR)

        y += 1.5 * height
        self.multiplayer_button = pygame.Rect(x, y, width, height)
        self.multiplayer_button_surface = \
            make_button_surface(self.button_font, self.multiplayer_button, "Битва с игроком", BUTTON_COLOR)

        y += 1.5 * height
        self.quit_button = pygame.Rect(x, y, width, height)
        self.quit_button_surface = \
            make_button_surface(self.button_font, self.quit_button, "Выйти", BUTTON_COLOR)
    ...
    def __make_top(self):
        result = pygame.Surface((settings.WINDOW_WIDTH, settings.WINDOW_HEIGHT), pygame.SRCALPHA, 32)
        text_surface = self.header_font.render("Шахматы", True, pygame.Color('#FFFFFF'))
